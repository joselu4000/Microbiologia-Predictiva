...
# Lectura de datos
file_path <- "datos/Datos2.xlsx"  # Cambia este path según sea necesario
datos <- read_excel(file_path)
data <- datos[16:nrow(sdf), 1:2]
data <- na.omit(data)
x <- data$x
y <- data$y
...
plot(x, y, col = "red", lwd = 2, 
     ...
     main = "Baranyi ode-Adam Datos 2")
lines(out[, "time"], out[, "y"], col = "black", lwd = 2)

# Calcular R^2 ajustado
out <- ode(y = state, times = x, func = model, parms = optimized_params)
y_pred <- out[, "y"]
SSE <- sum((y - y_pred)^2)
SST <- sum((y - mean(y))^2)
R2 <- 1 - (SSE / SST)
