...
error_function_direct_y <- function(params, t_data, y_data) {
  ...
  sol <- ode(y = state, times = times, func = model_direct_y, parms = parameters, method = "ode45")
...
initial_params_with_y0 <- c(min(y_data), 1, 400, max(y_data))

# Ajustar los límites de los parámetros
lower_bounds <- c(0, min(t_data), min(t_data), max(y_data) * 0.2)
upper_bounds <- c(3, 400, 500, max(y_data) * 2)
...
sol_final <- ode(y = state, times = times, func = model_direct_y, 
                          parms = parameters, method = "ode45")
sol_final <- as.data.frame(sol_final)
# Graficar resultados
...
plot(x,y, type="p", col="red",lwd=2,
     ...
     ylim = c(0,max(y)+1),
     main = "Trifase ode-L-BFGS-B Datos 2")
lines(sol_final$time,sol_final$y, col = "black", lwd = 2)
lines(rep(parameters[2],2),c(0,parameters[1]),lty = 2, col = "blue")
lines(rep(parameters[3],2),c(0,parameters[4]),lty = 2, col = "blue")
...
cat(sprintf("y0 = %.3f, t_lag = %.3f, t_max = %.3f, y_max = %.3f, MSE = %4.f\n", 
